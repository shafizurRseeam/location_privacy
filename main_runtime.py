...
def process_files_in_directory(directory_path, noise_samples_laplace, runs):
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.csv'):
            ...
            time_taken = apply_perturbation_and_measure_time(df, noise_samples_laplace, runs)
            total_time += time_taken
            file_count += 1
    if file_count > 0:
        average_time_per_file_across_all_runs = total_time / file_count
        print(f"Total time: {total_time:.4f} seconds, Average time per file across all runs: {average_time_per_file_across_all_runs:.4f} seconds")
    else:
...
def main():
    ...
    noise_samples_laplace = generate_laplace_noise_samples(10, args.epsilon)  # Adjust n_samples as necessary
    print(f"Processing CSV files in the current directory: {current_directory}")
    process_files_in_directory(current_directory, noise_samples_laplace, args.runs)
